...
const loginUser=async(req,res)=>{
    ...
    if(!email || !password) return res.status(422).json({error:'Some fields are missing'})
    if(!validator.isEmail(email)) return res.status(422).json({error:'Please provide a valid email'})
    const isExistingUser=await USER.findOne({email:email})
    if(!isExistingUser) return res.status(404).json({error:'User not found, please register'})
    const matchPassword=await bcrypt.compare(password,isExistingUser.password)
    if(!matchPassword) return res.status(400).json({error:'Passwords do not match'})
    const token=jwt.sign({_id:isExistingUser._id,email:email,name:isExistingUser.name,phoneNumber:isExistingUser.phoneNumber},process.env.SECRET_KEY,{expiresIn:'1d'})
    if(!token) return res.status(500).json({error:'Could not generate token'})
    const {name,phoneNumber,wishList,cartItems,address,profilePic}=isExistingUser
    return res.status(200).json({token,userData:{email,name,phoneNumber,wishList,cartItems,address,profilePic}})
}
